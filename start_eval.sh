...
# 计时开始
echo -e "${YELLOW}=== 开始启动评测程序（非沙箱模式） ===${NC}"
START_TIME=$(date +%s)
...
# 保存结果到日志文件
LOG_FILE="eval_log_$(date +%Y%m%d_%H%M%S).txt"
echo "=== 评测日志（非沙箱模式） ===" > "$LOG_FILE"
